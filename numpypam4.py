...
FIGURE_PATH = os.path.join(BASE_DIR, 'results_pam4_numpy', 'figure')
...
def get_snr_pam4_numpy(ref, rec):
    """专门为 PAM4 适配的 SNR 计算函数 (只有 0 和 180 度的相位模糊)"""
    ...
    for shift in range(-5, 6):
        ...
        for angle in [0, np.pi]: # PAM4 是一维信号，只有正负翻转
...
def main():
    ...
    print("Step 1: DP-PAM4 Signal Generation (NumPy)...")
    
    num_syms = 32768
    baud_rate = 32e9
    
    # 【修复 1】：显式声明为复数类型 (dtype=complex)
    pam_levels = np.array([-3, -1, 1, 3], dtype=complex)
    const = pam_levels / np.sqrt(np.mean(np.abs(pam_levels)**2))
    
    np.random.seed(42)
    tx_indices = np.random.randint(0, 4, (num_syms, 2))
    ...
    print("Step 3: Rx DSP (CDC + LMS + CPR)...")
    rx_2sps = apply_cd_numpy(rx_2sps, baud_rate, sps=2, D=17.0, L=-80.0)
    ...
    print("   Running DA/DD-LMS Equalizer (Slow Python Loop)...")
    rx_eq = pam4_lms_equalizer_numpy(rx_1sps, tx_syms, const, taps=3, train_len=5000)
    ...
    for i in range(2):
        rx_cpr[:, i] = cpr_4th_power_numpy(rx_eq_cut[:, i], mu=0.01)

    print("Step 4: Evaluation & Alignment...")
    aligned_list = []
    snr_vals = []
    for i in range(2):
        snr_v, aligned_sig = get_snr_pam4_numpy(tx_final[:, i], rx_cpr[:, i])
    ...
    ax1.set_title("1. After DA/DD-LMS (NumPy)")
    ax1.axis('equal')
    ...
    ax2.scatter(const.real, np.zeros_like(const.real), c='red', marker='x', s=40, alpha=0.9) 
    ...
    for i in range(2):
        ...
        ax.scatter(const.real, np.zeros_like(const.real), c='red', marker='x', s=40, alpha=0.9)
    ...
    plot_eye(rx_final, sps=1, name='Eye After DSP (Recovered)', amplitude_limit=2.0, filename='05_eye_after_dsp.png')

    end_time = time.time()
    
    print("\n" + "="*55)
    print(f"NumPy DP-PAM4 仿真完美结束！")
