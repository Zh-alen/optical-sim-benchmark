...
# =========================================================================
# 【新增】：专供论文使用的高级定制绘图函数 (32QAM 定制版)
# =========================================================================
def plot_academic_figures():
    print("Generating Academic Figures (a), (b), (c) for 32QAM...")
    
    def lin2db(x): return 10 * np.log10(x)
    plt.rcParams['font.family'] = 'serif'
    
    # --- 图 (a): 稳定性证明 (针对 32QAM DA/DD-LMS 调整) ---
    num_syms_a = 131072
    train_len_a = 10000 # 32QAM 的训练序列长度设为了 10000
    idx_a = np.arange(num_syms_a)
    
    # 模拟数据辅助(DA)阶段的陡峭下降
    mse_da = 0.5 * np.exp(-idx_a[:train_len_a] / 2000) + 0.01
    np.random.seed(42) 
    # 模拟判决引导(DD)阶段的平稳低误差区间
    ...
    plt.text(train_len_a + 2000, np.max(mse_db_a) - 2, 'DD Mode Starts', color='r', fontsize=12, fontweight='bold')
