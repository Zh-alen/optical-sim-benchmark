...
def _():
    ...
    mo.md(
        """
        # Coherent DP-PAM4 Optical Communication System with JAX High-Speed ​​LMS Equalization

    Welcome to the interactive simulation experiment of optical communication using pure one-dimensional real modulation (PAM4)!

    This tutorial demonstrates how to use the **data-driven + hard-decision hybrid (DA/DD-LMS)** algorithm.
        """
    )
    return (mo,)
...
def _(get_safe_rrc_taps, jax, jnp, np, sp_signal, sym_map):
    ...
    _tx_indices = jax.random.randint(_key, (num_syms, 2), 0, 4)
...
def _(
    ...
    # 画图：PAM4 特有的 3个眼孔 眼图
    fig_eye_pam4, _ax_eye = plt.subplots(1, 2, figsize=(10, 4))
    for _i in range(2):
...
@app.cell
def _(const, jnp, mo, pam4_lms_equalizer, rx_mf_aligned, tx_syms):
    mo.md("**正在执行 JAX 加速的 DA/DD-LMS ...**")

    _rx_1sps = rx_mf_aligned[::2] 
    _rx_1sps = _rx_1sps / jnp.sqrt(jnp.mean(jnp.abs(_rx_1sps)**2, axis=0))

    # 调用 JIT 编译的 LMS 均衡器
    _rx_eq = pam4_lms_equalizer(_rx_1sps, tx_syms, const, taps=3)
...
def _(CPR, ak, jax, jnp, rx_eq_cut):
    ...
    for _pol_idx in range(2):
        _cpr_module = CPR(kernel=ak.cpr_4thpower_pll(mu=0.01)) 
...
def _(const, get_ber, get_snr, jnp, mo, plt, rx_cpr_all, rx_eq_cut, tx_final):
    ...
    *Note: Modern coherent optical communication systems typically require a Pre-FEC (Forward Error Correction) BER threshold of $2 \\times 10^{{-2}}$ for error-free transmission.*
    ...
    _ax2.axis('equal')
    # 严格限制 Y 轴视角，凸显 PAM4 一维线状特性
    _ax2.set_ylim([-0.5, 0.5]) 
    _ax2.scatter(const.real, jnp.zeros_like(const.real), c='red', marker='x', s=40, alpha=0.9) 
    ...
    _ax3.set_title("3. PLL Phase Track Residual")
