...
def cpr_4thpower_pll(
    ...
    Constellation offset correction:
        For non-constant-modulus constellations (e.g. 16-QAM), E[s^M] has
        a non-zero phase that biases the PLL by angle(E[s^M])/M.  Pass
        ``const`` to auto-correct this.  Without correction, 16-QAM
        produces a deterministic π/4 tilt (45°, 135°, 225°, or 315°
        instead of 0°, 90°, 180°, 270°) because E[s⁴] = −0.68
        (phase = π), so dividing by 4 shifts the estimate by π/4.

    Args:
        mu: Loop bandwidth / learning rate.
            Larger = faster tracking, more noise.
            Typical: 0.001-0.1
        M: Power to raise signal (4 for square QAM).
            Removes modulation: symbol phases become multiples of 2π/M.
        const: Reference constellation (e.g. sym_map.const('16QAM')).
            If provided, angle(mean(const^M)) is used to correct the
            constellation-dependent phase bias.  If None, no correction
            is applied (correct for unit-amplitude QPSK with M=4).

    Returns:
        AdaptiveFilter with single-dimensional state.

    Example:
        # QPSK (no offset needed)
        cpr = eq.CPR(kernel=ak.cpr_4thpower_pll(mu=0.01))

        # 16-QAM (offset correction via const)
        const = sym_map.const('16QAM', norm=True)
        cpr = eq.CPR(kernel=ak.cpr_4thpower_pll(mu=0.01, const=const))
    """
    # Constellation-dependent phase offset: angle(E[s^M])
    if const is not None:
        const = jnp.asarray(const)
        phase_offset = jnp.angle(jnp.mean(const ** M))
    else:
        phase_offset = 0.0
    ...
    def update(i, phase, y):
        # M-th power removes modulation for constant-modulus constellations.
        # For multi-ring constellations (16-QAM), E[s^M] has non-zero phase
        # which is compensated by phase_offset in the expected term.
        y_M = y ** M

        # Expected phase after M-th power (includes constellation offset)
        expected = M * phase + phase_offset

        # Phase error (wrapped to [-π, π])
        err = jnp.angle(y_M) - expected
        err = jnp.arctan2(jnp.sin(err), jnp.cos(err))
