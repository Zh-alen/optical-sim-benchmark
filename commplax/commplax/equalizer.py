...
- **PilotCPR**: Pilot-aided Carrier Phase Recovery with non-causal interpolation.
  Uses known pilot symbols and a SyncFIFO delay line for exact linear
  interpolation between adjacent pilot phase estimates. Preferred over
  blind CPR for pilot-aided systems (e.g., 400G/800G coherent).
...
from commplax.buffer import SyncFIFO
...
class PilotCPR(eqx.Module):
    """Pilot-aided Carrier Phase Recovery with non-causal interpolation.

    Uses known pilot symbols and a SyncFIFO delay line to perform exact
    linear interpolation of carrier phase between adjacent pilot positions.
    The FIFO introduces P-symbol latency (where P = pilot_interval), but
    enables true interpolation rather than extrapolation.

    Architecture::

        symbol --> SyncFIFO(P) --> delayed_symbol --> phase_correct --> output
                       |
        pilot_tx --> phase_est --> interpolate(prev, curr, frac) -->

    At each pilot boundary (counter == 0):
      - Current input is a pilot: extract carrier phase via angle(rx * conj(tx))
      - Shift: prev_phase <- curr_phase, curr_phase <- new estimate
      - The P delayed symbols span [prev_pilot, curr_pilot]
      - Each gets phase correction linearly interpolated by counter/P

    Carry state for lax.scan:
        fifo:         SyncFIFO(P) -- P-symbol delay line
        prev_phasor:  complex -- unit phasor at the older pilot
        curr_phasor:  complex -- unit phasor at the newer pilot
        counter:      int in [0, P) -- position within pilot interval

    Args:
        pilot_interval: Number of symbols between pilots (default: 64)
        pilot_avg: Recursive averaging depth for pilot phase estimates
            (default: 1, no averaging).  At each pilot boundary the new
            phase estimate is smoothed with IIR gain alpha = 1/pilot_avg:
            ``smoothed = curr + alpha * unwrap(raw - curr)``.
            Higher values reduce pilot noise variance (~1/(2*SNR*pilot_avg))
            at the cost of slower tracking of fast phase changes.
        dtype: Complex dtype for signal processing (default: complex64)

    Returns:
        When called with (symbol, pilot_tx):
        (updated_module, corrected_delayed_symbol)

    Example:
        Dual-polarization pilot CPR::

            from commplax.equalizer import PilotCPR
            from commplax import module as mod

            @eqx.filter_vmap
            def make_cpr(_):
                return PilotCPR(pilot_interval=64)

            cpr = make_cpr(jnp.arange(2))

            @eqx.filter_vmap
            def cpr_step(cpr, inputs):
                symbol, pilot_tx = inputs
                return cpr(symbol, pilot_tx)

            scanner = mod.scan_with(cpr_step, jit_backend='cpu')
            cpr_final, rx_cpr = scanner(cpr, (rx, pilot_tx_all))
            rx_cpr_valid = rx_cpr[pilot_interval:]  # trim FIFO fill

    Note:
        - Input pilot_tx should be the known TX pilot at pilot positions
          and zero at data positions; only used when counter == 0.
        - The first pilot_interval outputs are from the FIFO fill period
          (zeros) and should be discarded.
        - Phase tracking uses trig-free complex phasor arithmetic
          (chord interpolation on the unit circle).
        - No pi/2 ambiguity (unlike blind 4th-power CPR).
    """
    fifo: SyncFIFO
    prev_phasor: Array
    curr_phasor: Array
    counter: Array
    pilot_interval: int = field(static=True)
    pilot_avg: int = field(static=True)

    def __init__(self, pilot_interval=64, pilot_avg=1, dtype=None,
                 fifo=None, prev_phasor=None, curr_phasor=None,
                 counter=None):
        dtype = default_complexing_dtype() if dtype is None else dtype
        self.pilot_interval = pilot_interval
        self.pilot_avg = pilot_avg
        self.fifo = SyncFIFO(pilot_interval, dtype=dtype) if fifo is None else fifo
        self.prev_phasor = jnp.array(1.0 + 0j, dtype=dtype) if prev_phasor is None else prev_phasor
        self.curr_phasor = jnp.array(1.0 + 0j, dtype=dtype) if curr_phasor is None else curr_phasor
        self.counter = jnp.array(0) if counter is None else counter

    def __call__(self, symbol, pilot_tx):
        """Process one symbol through pilot-aided CPR.

        Args:
            symbol:   received complex symbol
            pilot_tx: known TX pilot value at pilot positions,
                      ignored at data positions (counter != 0)
        ...
        # Push into FIFO, get P-delayed symbol
        fifo_new, delayed = self.fifo(symbol)
        delayed = delayed[0]  # SyncFIFO returns (1,) array

        # Pilot boundary: counter == 0
        is_pilot = self.counter == 0

        # Phase extraction: complex phasor instead of angle()
        ref = symbol * jnp.conj(pilot_tx)
        ref_phasor = ref / (jnp.abs(ref) + 1e-30)

        # IIR smoothing in complex domain
        alpha = 1.0 / self.pilot_avg
        smoothed = (1 - alpha) * self.curr_phasor + alpha * ref_phasor
        smoothed = smoothed / (jnp.abs(smoothed) + 1e-30)

        # Update phasors at pilot boundaries
        prev_phasor_new = jnp.where(is_pilot, self.curr_phasor, self.prev_phasor)
        curr_phasor_new = jnp.where(is_pilot, smoothed, self.curr_phasor)

        # Interpolate phasors on unit circle (chord ~ arc for small angles)
        frac = self.counter / P
        phasor_interp = (1 - frac) * prev_phasor_new + frac * curr_phasor_new
        phasor_interp = phasor_interp / (jnp.abs(phasor_interp) + 1e-30)

        # Correct delayed symbol: conj multiply instead of exp(-j*phase)
        corrected = delayed * jnp.conj(phasor_interp)

        counter_new = (self.counter + 1) % P

        cpr_new = dc.replace(
            self,
            fifo=fifo_new,
            prev_phasor=prev_phasor_new,
            curr_phasor=curr_phasor_new,
            counter=counter_new)
        return cpr_new, corrected
